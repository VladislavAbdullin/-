...
if($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if(isset($_POST['addToCart'])) {
        ...
        if($numExistRows > 0){
            echo "<script>alert('Товар уже добавлен.');
    ...
    if(isset($_POST['checkout'])) {
        ...
        if (password_verify($password, $passRow['password'])){ 
            ...
            if ($result){
                ...
                echo '<script>alert("Спасибо, что сделали заказ у нас. Ваш код заказа - это ' .$orderId. '.");
                    window.location.href="http://localhost/OnlineProductDelivery/index.php";  
                    </script>';
                    exit();
            }
        } 
        else{
            echo '<script>alert("Неверный пароль! Пожалуйста, введите правильный пароль.");
                    window.history.back(1);
                    </script>';
                    exit();
        }    
